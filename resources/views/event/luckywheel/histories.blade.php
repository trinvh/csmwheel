	...
	<div class="row">
		<div class="col-md-12">
			<div class="panel panel-default">
				<div class="panel-heading">Vòng quay may mắn - Xem lịch sử quay số
					<div class="btn-group btn-group-sm pull-right">
						<a href="{{ url('event/lucky-wheel/payment') }}" class="btn btn-default">Xem lịch sử nạp tiền</a>
						<a href="{{ url('event/lucky-wheel/histories') }}" class="btn btn-default active" >Lịch sử quay</a>
					</div>
				</div>
				<div class="panel-body">
					...
					<div class="col-md-8">
						<h3>Lịch sử trúng thưởng <small>vòng quay may mắn</small></h3>
						<hr>
						<div class="table-responsive">
							<table class="table">
								<tr>
									<th>Trúng / Không trúng</th>
									<th>Giải thưởng</th>
									<th>Thời gian</th>
								</tr>
								@foreach($histories as $history)
								<tr>
									<td>{!! (!$history->is_win) ? '<span class="label label-default">Không trúng</span>' : '<span class="label label-success">Trúng</span>' !!}</td>
									<td>@if($history->prize > 0) <span class="label label-danger">{{ $history->prize }}</span> @else 0 @endif</td>
